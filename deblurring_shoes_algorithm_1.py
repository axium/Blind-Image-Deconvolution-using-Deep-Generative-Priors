...
# paths
Orig_Path       = './results/Shoes/Original Images/*.png'
Range_Path      = './results/Shoes/Range Images/*.png'
Blur_Path       = './results/Shoes/Original Blurs/Test Blurs.npy'
...
# loading shoe generator
ShoeGen = ShoeGenerator()
ShoeGen.GenerateModel()
ShoeGen.LoadWeights()
ShoeGAN = ShoeGen.GetModels()
shoe_latent_dim = ShoeGen.latent_dim
...
image_gradients, blur_gradients, get_loss = Generate_Gradient_Functions(rr = Blurry_Images.shape[0],
                                                                        ...
                                                                        decoder = ShoeGAN, blur_decoder = blur_decoder,
...
m_hat, h_hat, Loss = Optimize_Parallel(blurry_fourier = Blurry_Images, stepsize=step_size,steps = STEPS,
                                      ...
                                      getloss = get_loss, latent_image_dim = shoe_latent_dim , latent_blur_dim = blur_latent_dim)
X_hat_test = []
W_hat_test = []
for i in range(len(X_Orig)):
    ...
    x_hat_test, w_hat_test, loss_last_iter_test = Get_Min_Loss(Loss_i, m_hat_i, h_hat_i, decoder = ShoeGAN, blur_decoder = blur_decoder,
                                                               latent_image_dim = shoe_latent_dim, latent_blur_dim = blur_latent_dim,  print_grad=False)  
...
for i in range(len(X_Orig)):
    ...
    h_hat_i = h_hat[i*RANDOM_RESTARTS:(i+1)*RANDOM_RESTARTS]
    Loss_i  = Loss[i*RANDOM_RESTARTS:(i+1)*RANDOM_RESTARTS]
    x_hat_range, w_hat_range, loss_last_iter_range = Get_Min_Loss(Loss_i, m_hat_i, h_hat_i, decoder = ShoeGAN, blur_decoder = blur_decoder,
                                                                  latent_image_dim = shoe_latent_dim, latent_blur_dim = blur_latent_dim, print_grad=False)  
