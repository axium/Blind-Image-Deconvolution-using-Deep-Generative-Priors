...
SAVE_PATH       = './results/Shoes/deblurring - alg2 - '+str(int(NOISE_STD*100)) + 'perc noise - ' +str(RANDOM_RESTARTS) + 'RR/deblurring_'
PLOT_LOSS       = True
SAVE_RESULTS    = True
# -----------------------------------------------------------------------
...
zi_tf = tf.Variable(tf.random_normal(shape=([rr, shoe_latent_dim])), dtype = 'float32')
...
for i in range(len(X_Orig)):
    ...
    zi_hat_recov = zi_hat_i[min_loss_loc].reshape([1,shoe_latent_dim])
    ...
    xg_hat = ShoeGAN.predict(zi_hat_recov).reshape(IMAGE_RES,IMAGE_RES,CHANNELS)
